...
EvaluatorResult evaluate(EvaluatorState* state, ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case NODE_VARIABLE:  // CORREÇÃO: NODE_VARIABLE em vez de NODE_IDENTIFIER
            if (variable_exists(state, node->variable)) {  // CORREÇÃO: node->variable
                return create_success_result(get_variable(state, node->variable), 0);  // CORREÇÃO: node->variable
            } else {
        ...
        case NODE_BINARY_OP:
            {
                ...
                switch (node->operator) {
                    ...
                    default: return create_error_result("Operador binário inválido");
                }
        ...
        case NODE_UNARY_OP:
            {
                EvaluatorResult operand_result = evaluate(state, node->operand);  // CORREÇÃO: node->operand
                ...
                switch (node->operator) {
                    ...
                    default: return create_error_result("Operador unário inválido");
                }
                return create_success_result(result, 0);
            }
            
        case NODE_FUNCTION:  // CORREÇÃO: NODE_FUNCTION em vez de NODE_FUNCTION_CALL
            {
                // CORREÇÃO: Usar node->args em vez de lista encadeada
...
EvaluatorResult execute_function(EvaluatorState* state, const char* function_name, 
    ...
    // CORREÇÃO: Já recebemos os valores convertidos, não precisa converter novamente!
    ...
    if (strcmp(function_name, "sqrt") == 0 || strcmp(function_name, "raiz") == 0) {
        if (arg_count != 1) {
            snprintf(error_msg, sizeof(error_msg), "sqrt requer 1 argumento");
        ...
        if (arg_count != 1) {
            snprintf(error_msg, sizeof(error_msg), "sin requer 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 1) {
            snprintf(error_msg, sizeof(error_msg), "cos requer 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 1) {
            snprintf(error_msg, sizeof(error_msg), "tan requer 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 1) {
            snprintf(error_msg, sizeof(error_msg), "log requer 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 1) {
            snprintf(error_msg, sizeof(error_msg), "ln requer 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 1) {
            snprintf(error_msg, sizeof(error_msg), "exp requer 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 1) {
            snprintf(error_msg, sizeof(error_msg), "abs requer 1 argumento");
            return create_error_result(error_msg);
        }
    ...
    // ============ FUNÇÕES ESTATÍSTICAS ============
        ...
        if (arg_count < 1) {
            snprintf(error_msg, sizeof(error_msg), "mean requer pelo menos 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 1) {
            snprintf(error_msg, sizeof(error_msg), "median requer pelo menos 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 1) {
            snprintf(error_msg, sizeof(error_msg), "std requer pelo menos 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 1) {
            snprintf(error_msg, sizeof(error_msg), "variance requer pelo menos 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 1) {
            snprintf(error_msg, sizeof(error_msg), "mode requer pelo menos 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 1) {
            snprintf(error_msg, sizeof(error_msg), "sum requer pelo menos 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 1) {
            snprintf(error_msg, sizeof(error_msg), "min requer pelo menos 1 argumento");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 1) {
            snprintf(error_msg, sizeof(error_msg), "max requer pelo menos 1 argumento");
            return create_error_result(error_msg);
        }
    ...
    // ============ FUNÇÕES FINANCEIRAS ============
        ...
        if (arg_count != 3) {
            snprintf(error_msg, sizeof(error_msg), "pv requer 3 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 3) {
            snprintf(error_msg, sizeof(error_msg), "fv requer 3 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 3) {
            snprintf(error_msg, sizeof(error_msg), "pmt requer 3 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 3) {
            snprintf(error_msg, sizeof(error_msg), "nper requer 3 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 4) {
            snprintf(error_msg, sizeof(error_msg), "rate requer 4 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 3) {
            snprintf(error_msg, sizeof(error_msg), "si requer 3 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 3) {
            snprintf(error_msg, sizeof(error_msg), "fv_si requer 3 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 3) {
            snprintf(error_msg, sizeof(error_msg), "ci requer 3 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count != 3) {
            snprintf(error_msg, sizeof(error_msg), "fv_ci requer 3 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 2) {
            snprintf(error_msg, sizeof(error_msg), "npv requer pelo menos 2 argumentos");
            return create_error_result(error_msg);
        }
        ...
        if (arg_count < 2) {
            snprintf(error_msg, sizeof(error_msg), "irr requer pelo menos 2 argumentos");
            return create_error_result(error_msg);
        }
        ...
        snprintf(error_msg, sizeof(error_msg), "Função desconhecida: %s", function_name);
        return create_error_result(error_msg);
    }
    
    // Verifica se resultado é NAN
    if (isnan(result)) {
        snprintf(error_msg, sizeof(error_msg), "Erro matemático na função %s", function_name);
        return create_error_result(error_msg);
    }
