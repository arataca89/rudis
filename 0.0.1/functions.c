 ...
 * Retorna NAN para números negativos ou não inteiros.
 */
...
double math_nper(double rate, double pmt, double pv) {
    ...
    double result = log(1 - (pv * rate) / pmt) / log(1 + rate);
...
double math_rate(double nper, double pmt, double pv, double fv) {
    ...
    double rate = 0.1;  // Chute inicial de 10%
    double precision = 1e-8;
    int max_iterations = 50;
    
    for (int i = 0; i < max_iterations; i++) {
        double f = pv * pow(1 + rate, nper) + pmt * (pow(1 + rate, nper) - 1) / rate + fv;
        
        // Derivada numérica (mais estável)
        double delta = rate * 1e-6;
        if (fabs(rate) < 1e-12) delta = 1e-6; // Evita divisão por zero
        
        double f_plus = pv * pow(1 + rate + delta, nper) + 
                       pmt * (pow(1 + rate + delta, nper) - 1) / (rate + delta) + fv;
        double f_derivative = (f_plus - f) / delta;
        
        if (fabs(f_derivative) < precision) break;
        
        double new_rate = rate - f / f_derivative;
        
        // Limita a taxa para evitar valores absurdos
        if (new_rate < -0.9) new_rate = -0.9;
    ...
    return NAN;  // Retorna NAN se não convergir
}
*/
