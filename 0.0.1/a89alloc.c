...
void* a89alloc(size_t size, const char* file, int line) {
    ...
    if (ptr != NULL) {
        ...
        //printf("ALOCAÇÃO: %zu bytes em %s:%d (ptr: %p)\n", 
        //       size, file, line, ptr);
    } else {
...
void a89check_leaks(void) {
    if (total_allocations == 0) {
        printf("SUCESSO: Nenhum vazamento de memória detectado!\n");
        printf("   Todas as alocações foram devidamente liberadas.\n");
        return;
    }
    
    // Cabeçalho do relatório de vazamentos
    printf("VAZAMENTOS DETECTADOS: %d alocação(ões) não liberada(s)\n", 
           total_allocations);
    ...
    for (int i = 0; i < total_allocations; i++) {
        ...
        if (allocations[i].size > 1024) {
            ...
            printf("  IMPACTO MÉDIO: Vazamento > 100 bytes\n");
        }
    ...
    printf("  Média por vazamento: %.2f bytes\n", 
    ...
    // Recomendações
    printf("\nRECOMENDAÇÕES:\n");
    printf("  1. Revisar cada localização listada acima\n");
    printf("  2. Adicionar chamadas a89free() correspondentes\n");
    printf("  3. Verificar fluxos de controle (returns, gotos)\n");
    printf("  4. Considerar uso de ferramentas automáticas de análise\n");
}


void a89report_alloc(void) {
    printf("RELATÓRIO DE ALOCAÇÕES ATIVAS\n");
    ...
    printf("Total de alocações ativas: %d\n", total_allocations);
    
    if (total_allocations == 0) {
        printf("Nenhuma alocação ativa no momento.\n");
        printf("   Sistema de memória limpo.\n");
    ...
    // Resumo estatístico
    printf("\nESTATÍSTICAS GERAIS:\n");
    printf("  Total de memória alocada: %zu bytes (%.2f KB)\n", 
           total_memory, (double)total_memory / 1024.0);
    printf("  Média por alocação: %.2f bytes\n", 
           (double)total_memory / total_allocations);
    printf("  Menor alocação: %zu bytes\n", min_alloc);
    printf("  Maior alocação: %zu bytes\n", max_alloc);
